    ...
    for use_rtl in [False]:  # False=HLS
        ...
        try:
            result = run_build(
                ...
                hw_efficient=hw_efficient, low_latency_accu=low_latency_accu
            )
...
def main():
    ...
    parser.add_argument(
        "--hw-efficient",
        action="store_true",
        help="Use LUT-efficient VersalAtom222 cascade (O52->I4 ripple)",
    )
    parser.add_argument(
        "--low-latency-accu",
        action="store_true",
    ...
    # 14 configs from versal_full_dotp.csv (dotp_comp path, WW/AW <= 4)
    # (MW, MH, PE, SIMD, WW, AW)
    configs = [
        # PE=1 sweep
        (32, 18, 1, 1, 2, 2),
    ...
    print(f"hw_efficient: {args.hw_efficient} (VersalAtom222 cascade)")
    print(f"low_latency_accu: {args.low_latency_accu} (pipelined quad + binary)")
