...
def run_build(model_path, output_dir, board, use_rtl, synth_only):
    ...
    cfg = DataflowBuildConfig(
        ...
        synth_clk_period_ns=1.5,  # Aggressive target to force pipelining
        board=board,
...
def run_comparison(mw, mh, pe, simd, ww, aw, board, fpga_part, work_dir, synth_only, timing_search):
...
def main():
    ...
    for mw, mh, pe, simd, ww, aw in configs:
        ...
        label, results = run_comparison(
            mw, mh, pe, simd, ww, aw, board, fpga_part, work_dir, args.synth_only, args.timing_search
        )
